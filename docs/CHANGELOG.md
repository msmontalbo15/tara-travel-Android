...
> Last updated: **2026-09-13 00:59 PHT**
...
- **IMP-110** (Legal & Auth / Mandatory Terms Review & Redundancy Removal): Enforced mandatory Terms & NPC Privacy Policy (RA 10173) reading before account creation, locking "Create Account" until policy sheet is reviewed and accepted, and removed redundant landing screen terms link.
- **IMP-109** (Auth & UX / Logout Navigation Hardening & Seamless Consent): Centralized sign-out navigation authority in `AuthGate`, resolved splash screen stacking upon logout, and removed pre-auth friction for returning users.
- **IMP-108** (Auth & Android / Google Sign-In ApiException 10 Resolution): Registered debug SHA-1 OAuth 2.0 client ID in `google-services.json` and aligned build signing.
- **IMP-107** (Onboarding / Streamlined 5-Step Architecture & Pure Google Auth): Streamlined onboarding to 5 steps, merged photo/nickname/DOB into `PersonalProfileStep`, and removed MPIN/biometrics from onboarding.
