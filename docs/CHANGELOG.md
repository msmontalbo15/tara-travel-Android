...
- **IMP-113** (Auth & Onboarding / Onboarding Lifecycle Fix for Old & New Users): Prevented profile data wiping on Google sign-in by awaiting `refreshProfile()` before any state writes, eliminated premature onboarding bypass by removing `homeCity` heuristic from `isAccountFullySet` and `_loadProfile()`, fixed splash screen auto-routing for incomplete sessions, and added reactive `ref.listen` progress restoration in `OnboardingScreen`.
